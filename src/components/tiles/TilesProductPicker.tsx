...
// a one-result-at-a-time flow; this presents a two-column, shop-style product
// grid with direct add on every card instead.
    ...
    <Modal visible={open} animationType="slide" onRequestClose={onClose} presentationStyle="fullScreen">
      <SafeAreaView style={styles.screen} edges={["top", "bottom"]}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => <TileCatalogCard product={item} onPress={() => void pick(item)} />}
...
function TileCatalogCard({ product, onPress }: { product: Product; onPress: () => void }) {
  return (
    <Pressable onPress={onPress} style={({ pressed }) => [styles.card, pressed && styles.cardPressed]} testID={`tiles-picker-card-${product.id}`} accessibilityRole="button" accessibilityLabel={`Add ${product.name}`}>
      <ProductImage source={productImageList(product)} style={styles.image} fallbackLabel={product.sku} disableSkeleton />
...
const styles = StyleSheet.create({
  ...
  image: { width: "100%", aspectRatio: 1, borderRadius: 0, backgroundColor: colors.surfaceTertiary },
