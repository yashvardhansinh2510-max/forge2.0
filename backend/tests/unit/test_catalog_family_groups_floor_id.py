...
def test_family_groups_keep_a_standalone_product_visible(monkeypatch):
    class _StandaloneSnapshot:
        products = [{
            "id": "solo-1", "sku": "SOLO-1", "name": "Standalone Basin",
            "brand_id": "b1", "category_id": "c1", "price": 100, "mrp": 120,
            "images": [], "floor_id": "first-floor",
        }]
        media_rows_by_product = {}
        media_rows_by_family = {}

    async def _fake_snapshot():
        return _StandaloneSnapshot()
    ...
    assert result["items"][0]["family_key"] is None
    assert result["items"][0]["variants"][0]["sku"] == "SOLO-1"
