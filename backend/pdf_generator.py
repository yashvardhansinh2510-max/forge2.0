...
def _img(url: str | None, width_mm: float = 10, height_mm: float = 10) -> Flowable:
...
def _brand_header(right_title: str, styles: dict) -> Table:
    ...
    table = Table([[logo, Paragraph(right_title, styles["titleRight"])]], colWidths=[105 * mm, 65 * mm])
...
def build_quotation_pdf(quotation: dict, customer: dict, branding: dict | None = None) -> bytes:
    ...
    styles = {
        ...
        "body": ParagraphStyle("body", parent=base["Normal"], fontName="Helvetica", fontSize=8, leading=10.5, textColor=INK),
        "small": ParagraphStyle("small", parent=base["Normal"], fontName="Helvetica", fontSize=6.7, leading=8, textColor=INK),
        "tiny": ParagraphStyle("tiny", parent=base["Normal"], fontName="Helvetica", fontSize=6.5, leading=7.6, textColor=INK),
        "section": ParagraphStyle("section", parent=base["Normal"], fontName="Helvetica-Bold", fontSize=10, leading=12, textColor=INK),
        "cell": ParagraphStyle("cell", parent=base["Normal"], fontName="Helvetica", fontSize=6.6, leading=7.7, textColor=INK),
        "cellRight": ParagraphStyle("cellRight", parent=base["Normal"], fontName="Helvetica", fontSize=6.6, leading=7.7, textColor=INK, alignment=2),
        "tableHead": ParagraphStyle("tableHead", parent=base["Normal"], fontName="Helvetica-Bold", fontSize=6.5, leading=7, textColor=INK, alignment=1),
    ...
    summary_rows: list[list[object]] = [[
        Paragraph("SL. NO.", styles["tableHead"]), Paragraph("BATHROOM / AREA", styles["tableHead"]),
        Paragraph("MRP (Rs.)", styles["tableHead"]), Paragraph("OFFER RATE (Rs.)", styles["tableHead"]),
    ]]
    for index in range(8):
        room = room_order[index] if index < len(room_order) else ""
        room_items = grouped.get(room, []) if room else []
        gross, _, net = _room_totals(room_items)
        summary_rows.append([
            str(index + 1), Paragraph(_escape(room), styles["cell"]),
            Paragraph(_money(gross) if room else "", styles["cellRight"]),
            Paragraph(_money(net) if room else "", styles["cellRight"]),
        ])
    summary_rows.extend([
        ["", Paragraph("<b>TOTAL</b>", styles["cellRight"]), Paragraph(f"<b>{_money(quotation.get('subtotal', 0))}</b>", styles["cellRight"]), Paragraph(f"<b>{_money(quotation.get('grand_total', 0))}</b>", styles["cellRight"])],
        ["", Paragraph("<b>SPECIAL OFFER RATE</b>", styles["cellRight"]), "", Paragraph(f"<b>{_money(quotation.get('grand_total', 0))}</b>", styles["cellRight"])],
    ])
    summary = Table(summary_rows, colWidths=[17 * mm, 75 * mm, 39 * mm, 39 * mm], rowHeights=[6.5 * mm] + [5 * mm] * 8 + [5.8 * mm, 5.8 * mm])
    summary.setStyle(TableStyle([
        ("GRID", (0, 0), (-1, -1), 0.35, GRID), ("BACKGROUND", (0, 0), (-1, 0), HEADER_GREY),
        ("BACKGROUND", (0, 2), (-1, 2), ZEBRA), ("BACKGROUND", (0, 4), (-1, 4), ZEBRA),
        ("BACKGROUND", (0, 6), (-1, 6), ZEBRA), ("BACKGROUND", (0, 8), (-1, 8), ZEBRA),
    ...
    partner_rows = [[Paragraph(f"<b>{name}</b><br/><font size='5.4'><i>{tagline}</i></font>", ParagraphStyle("partner", parent=styles["cell"], alignment=1, leading=8)) for name, tagline in row] for row in partners]
    ...
    item_header = [
        ...
        Paragraph("MRP<br/>(Rs.)", styles["tableHead"]), Paragraph("Q<br/>T<br/>Y", styles["tableHead"]),
        Paragraph("OFFER<br/>RATE", styles["tableHead"]), Paragraph("TOTAL<br/>(Rs.)", styles["tableHead"]),
    ]
    item_widths = [12 * mm, 33 * mm, 20 * mm, 34 * mm, 20 * mm, 8 * mm, 21 * mm, 22 * mm]
    for area_index, room in enumerate(room_order, 1):
        room_items = grouped.get(room, [])
        blocks = [room_items[i:i + 16] for i in range(0, len(room_items), 16)] or [[]]
        for block_index, block in enumerate(blocks):
            story.append(PageBreak())
            area_label = f"AREA {area_index}: <u>{_escape(room)}</u>"
            if block_index:
                area_label += " <font size='8'>(continued)</font>"
            story.append(_brand_header(area_label, styles))
            ...
            for row_index in range(16):
                item = block[row_index] if row_index < len(block) else None
                if item is None:
                    rows.append([str(row_index + 1), Paragraph("<i><font color='#999999'>[image]</font></i>", styles["cell"]), "", "", "", "", "", ""])
                    continue
                qty = float(item.get("qty") or 0)
                offer_rate = float(item.get("unit_price") or 0)
                pct = float(item.get("discount_pct") or 0)
                line_total = qty * offer_rate * (1 - pct / 100)
                listed_mrp = float(item.get("mrp") or offer_rate)
                ...
                rows.append([
                    str(row_index + 1), _img(item.get("image")), Paragraph(_escape(item.get("sku")), styles["cell"]), Paragraph(description, styles["tiny"]),
                    Paragraph(_money(listed_mrp), styles["cellRight"]), Paragraph(f"{qty:g}", styles["cellRight"]),
                    Paragraph(_money(offer_rate), styles["cellRight"]), Paragraph(_money(line_total), styles["cellRight"]),
                ])
            block_net = sum(float(item.get("qty") or 0) * float(item.get("unit_price") or 0) * (1 - float(item.get("discount_pct") or 0) / 100) for item in block)
            rows.append(["", "", "", Paragraph("<b>TOTAL</b>", styles["cellRight"]), "", "", "", Paragraph(f"<b>{_money(block_net)}</b>", styles["cellRight"])])
            table = Table(rows, colWidths=item_widths, rowHeights=[9 * mm] + [13 * mm] * 16 + [7 * mm])
            table.setStyle(TableStyle([
                ("GRID", (0, 0), (-1, -1), 0.3, GRID), ("BACKGROUND", (0, 0), (-1, 0), HEADER_GREY),
                ("BACKGROUND", (0, 2), (-1, 2), ZEBRA), ("BACKGROUND", (0, 4), (-1, 4), ZEBRA),
                ("BACKGROUND", (0, 6), (-1, 6), ZEBRA), ("BACKGROUND", (0, 8), (-1, 8), ZEBRA),
                ("BACKGROUND", (0, 10), (-1, 10), ZEBRA), ("BACKGROUND", (0, 12), (-1, 12), ZEBRA),
                ("BACKGROUND", (0, 14), (-1, 14), ZEBRA), ("BACKGROUND", (0, 16), (-1, 16), ZEBRA),
