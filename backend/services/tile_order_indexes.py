...
async def ensure_tile_order_indexes() -> None:
    ...
    await db.customer_orders.create_index(
        [("floor_id", 1), ("is_deleted", 1), ("created_at", 1)],
        name="customer_order_floor_active_created",
    )
    ...
    await db.purchase_orders.create_index(
        [("floor_id", 1), ("brand_id", 1), ("overall_status", 1), ("created_at", 1)],
        name="po_floor_brand_status_created",
    )
    await db.purchase_orders.create_index(
        [("floor_id", 1), ("customer_order_id", 1)],
        name="po_floor_customer_order",
    )
    ...
    await db.dispatches.create_index(
        [("floor_id", 1), ("customer_order_id", 1), ("is_deleted", 1)],
        name="dispatch_floor_customer_order_active",
    )
