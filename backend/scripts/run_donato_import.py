# backend/scripts/run_modulo_import.py
"""DONATO brand batch importer (Ground Floor > Tiles) — processes the single
DONATO 2026 pricelist. Safe to re-run: every row's SKU is deterministic
(name+size+finish, with a numeric suffix for genuine duplicate listings), so
re-running always upserts the the same 128 products instead of duplicating them.

Usage:
    python scripts/run_modulo_import.py --dry-run   # extract+validate+certify only, NO db writes
    python scripts/run_modulo_import.py              # full import (writes to Mongo + Supabase)
"""
...
from catalog_pipeline.adapters.donato import DonatoAdapter  # noqa: E402
...
SOURCE_FILE = REPO_ROOT / "backend" / "temp" / "donato_source_files" / "DONATO 2026.xlsx"
REPORT_PATH = REPO_ROOT / "memory" / "donato_qa_report.json"
