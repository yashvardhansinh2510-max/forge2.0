# backend/scripts/run_nexion_import.py
"""Nexion brand batch importer (Ground Floor > Tiles) — processes the single
NEXION 2026 pricelist. Safe to re-run: every row's SKU is deterministic
(name+size+finish), so re-running always upserts the same ~762 products
instead of duplicating them.

Usage:
    python scripts/run_nexion_import.py --dry-run   # extract+validate+certify only, NO db writes
    python scripts/run_nexion_import.py              # full import (writes to Mongo + Supabase)
"""
...
from catalog_pipeline.adapters.nexion import NexionAdapter  # noqa: E402
...
SOURCE_FILE = REPO_ROOT / "backend" / "temp" / "nexion_source_files" / "NEXION 2026.xlsx"
REPORT_PATH = REPO_ROOT / "memory" / "nexion_qa_report.json"
