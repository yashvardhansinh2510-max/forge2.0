"""GROHE Catalog ADDITIVE Batch 4 (2026-08) — 1 more supplier file.
...
categories/products). Files: Angle Valve.
...
Usage:
    python scripts/run_grohe_batch4_additive.py --dry-run
    python scripts/run_grohe_batch4_additive.py --execute
"""
...
from grohe_xlsx_extract import extract_all, FILES_BATCH4, ORIGINAL_FILENAMES_BATCH4, GroheRow  # noqa: E402
...
async def run(dry_run: bool) -> dict:
    ...
    print("=" * 78)
    print("BATCH 4 — EXTRACT & VALIDATE 1 SUPPLIER FILE")
    print("=" * 78)
    all_rows = extract_all(XLSX_DIR, FILES_BATCH4, ORIGINAL_FILENAMES_BATCH4)
    ...
    print(f"Existing Grohe products (batches 1+2+3, untouched by this run): {len(existing_products)}")
    ...
    print("\n" + "=" * 78)
    print("BACKUP (Grohe-only snapshot BEFORE adding batch 4)")
    print("=" * 78)
...
if __name__ == "__main__":
    ...
    lock_path = BACKUP_ROOT / "grohe_batch4.lock"
    if not dry:
        # Guard against the double-invocation incident from batch 3 (the tool
        # infra retried a "timed out" foreground call while the background
        # process was still genuinely running, causing a duplicate-insert).
        if lock_path.exists():
            print(f"!!! Lock file {lock_path} already exists — another execute run is in "
                  f"progress or a previous run crashed without cleanup. Refusing to run twice. "
                  f"Delete the lock file only after confirming no other run is active.")
            sys.exit(1)
    ...
    try:
        ...
        if not dry and lock_path.exists():
            lock_path.unlink()
    out_path = BACKUP_ROOT / "grohe_batch4_report.json"
