# backend/scripts/run_dimore_import.py
"""Dimore brand batch importer (Ground Floor > Tiles) — processes the single
DIMORE 2026 pricelist. Safe to re-run: every row's SKU is deterministic
(name+size+finish), so re-running always upserts the same ~256 products
instead of duplicating them.

Usage:
    python scripts/run_dimore_import.py --dry-run   # extract+validate+certify only, NO db writes
    python scripts/run_dimore_import.py              # full import (writes to Mongo + Supabase)
"""
...
from catalog_pipeline.adapters.dimore import DimoreAdapter  # noqa: E402
...
SOURCE_FILE = REPO_ROOT / "backend" / "temp" / "dimore_source_files" / "DIMORE 2026.xlsx"
REPORT_PATH = REPO_ROOT / "memory" / "dimore_qa_report.json"
