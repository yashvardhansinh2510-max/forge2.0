...
"""Modulo brand batch importer (Ground Floor > Tiles) — processes the single
MODULO 2026 pricelist. Safe to re-run: every row's SKU is deterministic
(name+size+finish, with a numeric suffix for genuine duplicate listings), so
re-running always upserts the same ~73 products instead of duplicating them.
...
from catalog_pipeline.adapters.modulo import ModuloAdapter  # noqa: E402
...
SOURCE_FILE = REPO_ROOT / "backend" / "temp" / "modulo_source_files" / "MODULO 2026.xlsx"
REPORT_PATH = REPO_ROOT / "memory" / "modulo_qa_report.json"
