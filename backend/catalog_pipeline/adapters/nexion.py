"""NEXION XLSX adapter (Ground Floor > Tiles).

Ships as a single workbook, single sheet, one row per (product name, size,
finish) combination:
  ...
  Row 2+ : one row per size/finish variant. The embedded product photo
           ...
           row anchoring (721 images across 762 real rows, zero rows with
           more than one image), so row+column anchor matching is safe here.

Same shape as Dimore: **no SERIES NAME column and no company-name column**
— PRODUCT NAME alone is the grouping key for sibling size/finish variants
(e.g. "CALACATTA" repeats across 8 finishes x 5 sizes, all one family).

Rules (mirrors the Qutone/Dimore adapters' documented conventions exactly,
adjusted for this source's actual shape — verified by direct inspection of
the real 762-row file before writing this):
* Brand is always "Nexion"; category is always "Tiles".
* No SKU/article code exists anywhere in the source. Verified unique across
  all 762 real rows: (PRODUCT NAME, PRODUCT SIZE, FINISHES). SKU is
  therefore synthesized, deterministic:
  `NEXION-{NAME_COMPACT}-{SIZE_COMPACT}-{FINISH_CODE}` — same inputs always
  regenerate the same SKU (required for idempotency). A repeated listing in
  a future refreshed file gets a numeric suffix (`-2`, `-3`, ...), matching
  the Qutone/Dimore/Oyster convention, so it still imports as its own
  product instead of being dropped.
* "FINISHES" is normalized via an explicit FINISH_LOOKUP table (not fuzzy
  regex) built from the 9 distinct raw strings actually observed in the
  real file (BOCCIRDATO, FULL LAPPATO, LAPPATO MATT, MATT, PATINATO,
  RIGATO, SATINATO, SOFT NATURAL, SPAZZOLATO). Anything new is flagged for
  manual review, never guessed.
* Family key groups sibling size/finish variants of the same product:
  `nexion:{name_slug}` (no series component — this source has none).
* "RATE" is the only price the source provides (format "<number> PER
  SQFT") — used for both `mrp` and `dealer_price`, since there is no
  separate MRP tier in this source (inventing one would be fabrication). A
  row whose RATE doesn't match the expected format is flagged
  `needs_pricing` and imported at ₹0 rather than held back. (Verified:
  762/762 real rows match the expected format — this path is a defensive
  fallback, not something the current file triggers.)
* Column positions are discovered from the header row text, not hardcoded
  ...
  columns in a different order, or with a SERIES NAME / company column
  added later.
* Images extracted with `optimize=False` — same original-quality
  requirement already established for Qutone/Dimore, reused here unchanged
  so all three Ground Floor Tiles brands behave identically.
"""
...
# Explicit lookup, not regex guessing — built from the 9 distinct raw
...
FINISH_LOOKUP: dict[str, tuple[str, str]] = {
    "BOCCIRDATO": ("Boccirdato", "BC"),
    "FULL LAPPATO": ("Full Lappato", "FL"),
    "LAPPATO MATT": ("Lappato Matt", "LM"),
    "MATT": ("Matt", "MT"),
    "PATINATO": ("Patinato", "PT"),
    "RIGATO": ("Rigato", "RG"),
    "SATINATO": ("Satinato", "ST"),
    "SOFT NATURAL": ("Soft Natural", "SN"),
    "SPAZZOLATO": ("Spazzolato", "SZ"),
}
...
class NexionAdapter(BrandAdapter):
    brand = BRAND
    ...
    def extract(self, data: bytes, filename: str) -> tuple[list[ProductRow], ExtractionReport]:
        ...
        for ws in wb.worksheets:
            ...
            # requirement is original quality, no resize/recompress (same as
            # Qutone/Dimore).
