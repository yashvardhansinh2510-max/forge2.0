"""DIMORE XLSX adapter (Ground Floor > Tiles).
...
finish) combination:
  ...
  Row 1  : column headers (SR., company NAME, PRODUCT NAME, IMAGE,
           PRODUCT SIZE, FINISHES, BOX IN PIS, BOX SQFT, RATE)
  Row 2+ : one row per size/finish variant. The embedded product photo
           ...
           row anchoring (all 256 real rows have exactly one image), so
           row+column anchor matching is safe here.

Unlike Qutone, this source has **no SERIES NAME column** — PRODUCT NAME
alone is the grouping key for sibling size/finish variants (e.g. "BARDIGLIO
BIANCO" repeats across 4 finishes x 2 sizes = 8 rows, all one family).

Rules (mirrors the Qutone adapter's documented conventions exactly, adjusted
for this source's actual shape — verified by direct inspection of the real
256-row file before writing this):
* Brand is always "Dimore"; category is always "Tiles".
* No SKU/article code exists anywhere in the source. Verified unique across
  all 256 real rows: (PRODUCT NAME, PRODUCT SIZE, FINISHES). SKU is
  therefore synthesized, deterministic:
  `DIMORE-{NAME_COMPACT}-{SIZE_COMPACT}-{FINISH_CODE}` — same inputs always
  ...
  the Qutone/Oyster convention, so it still imports as its own product
...
* "FINISHES" is normalized via an explicit FINISH_LOOKUP table (not fuzzy
  ...
  real file (LUMINOSO, OPACO NATURAL, SETADURA, BALANCIO, MATT, GRANULO
  NATURALE, PIETRA NATURALE, BRILLIO, FIGURA). Anything new is flagged for
  manual review, never guessed.
* Family key groups sibling size/finish variants of the same product:
  `dimore:{name_slug}` (no series component — this source has none).
* "RATE" is the only price the source provides (format "<number> PER
  ...
  256/256 real rows match the expected format — this path is a defensive
...
* Images extracted with `optimize=False` — same original-quality
  requirement as Qutone (this pipeline-wide opt-out was already added for
  that brand; reused here unchanged).
"""
...
FINISH_LOOKUP: dict[str, tuple[str, str]] = {
    "LUMINOSO": ("Luminoso", "LU"),
    "OPACO NATURAL": ("Opaco Natural", "ON"),
    "SETADURA": ("Setadura", "SD"),
    "BALANCIO": ("Balancio", "BL"),
    "MATT": ("Matt", "MT"),
    "GRANULO NATURALE": ("Granulo Naturale", "GN"),
    "PIETRA NATURALE": ("Pietra Naturale", "PN"),
    "BRILLIO": ("Brillio", "BR"),
    "FIGURA": ("Figura", "FG"),
}
...
class DimoreAdapter(BrandAdapter):
    brand = BRAND
