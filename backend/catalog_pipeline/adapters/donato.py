"""DONATO XLSX adapter (Ground Floor > Tiles).
...
Same shape as the Dimore/Modulo source (no SERIES NAME column — PRODUCT NAME alone
...
source's actual content (verified by direct inspection of the real 128-row
file before writing this):

* Brand is always "Donato"; category is always "Tiles" (per business rule —
  Donato belongs exclusively to the Ground Floor Tiles unit).
* No SKU/article code/barcode exists anywhere in the source. Verified
  unique across all 128 real rows: (PRODUCT NAME, PRODUCT SIZE, FINISHES, RATE) —
  ...
  SHA-1s for the source rows, zero shared). Treated as distinct face/pattern
  ...
  synthesized, deterministic: `DONATO-{NAME_COMPACT}-{SIZE_COMPACT}-
...
* "FINISHES" is normalized via an explicit FINISH_LOOKUP table (not fuzzy
  regex) built from the two distinct raw strings actually observed in the
  real file (GLOSSY, MATT). Anything new is flagged for manual review,
  never guessed.
* Family key groups sibling size/finish variants of the same product:
  `donato:{name_slug}` (no series component — this source has none).
* "RATE" is the only price the source provides, format "<number> PER
  SQFT" — observed source rates are 85, 110, 150, 155, 185, and 275 per
  sqft. Used for both `mrp` and
...
class DonatoAdapter(BrandAdapter):
    brand = BRAND
